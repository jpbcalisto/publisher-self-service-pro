...
function chip(format, group) {
  ...
  if (format.livePreviewUrl) {
    ...
    liveBtn.onclick = (e) => {
      e.stopPropagation();
      window.open(format.livePreviewUrl, '_blank');
    };
    button.appendChild(liveBtn);
  }
...
const opts = (sel, exclude=[]) => {
  // Ensure 'Others' is always present and last
  const sorted = [...countries].sort((a, b) => {
    if (a.code === 'Other' || a.code === 'OS') return 1;
    if (b.code === 'Other' || b.code === 'OS') return -1;
    return a.name.localeCompare(b.name);
  });
  // If no selection, default to 'OS' or 'Other'
  let selected = sel || (sorted.find(c => c.code === 'OS') ? 'OS' : 'Other');
  return sorted
    .filter(c => !exclude.includes(c.code))
    .map(c => `<option value="${c.code}" ${selected === c.code ? 'selected' : ''}>${c.name}</option>`).join('');
}
  geoSection.innerHTML = `
    ...
    <div class="geo-grid">
      <div class="geo-item">
        ...
        <div class="geo-row">
          <select class="geo-main"><option value="">Select</option>${opts(site.mainGeo, [site.secondaryGeo, site.otherGeo].filter(Boolean))}</select>
      ...
      <div class="geo-item">
        ...
        <div class="geo-row">
          <select class="geo-secondary"><option value="">Select</option>${opts(site.secondaryGeo, [site.mainGeo, site.otherGeo].filter(Boolean))}</select>
      ...
      <div class="geo-item">
        <label>Other</label>
        <div class="geo-row">
          <select class="geo-other"><option value="">Select</option>${opts(site.otherGeo, [site.mainGeo, site.secondaryGeo].filter(Boolean))}</select>
  ...
  const bind = () => {
    ...
    // Prevent duplicate GEO selection
    if (site.mainGeo && site.mainGeo === site.secondaryGeo) {
      site.secondaryGeo = '';
      if (secGeoEl) secGeoEl.value = '';
    }
    if (site.mainGeo && site.mainGeo === site.otherGeo) {
      site.otherGeo = (countries.find(c => c.code === 'OS') ? 'OS' : 'Other');
      if (otherGeoEl) otherGeoEl.value = site.otherGeo;
    }
    if (site.secondaryGeo && site.secondaryGeo === site.otherGeo) {
      site.otherGeo = (countries.find(c => c.code === 'OS') ? 'OS' : 'Other');
      if (otherGeoEl) otherGeoEl.value = site.otherGeo;
    }
    // Always default Other GEO to 'Others' if blank
    if (!site.otherGeo) {
      site.otherGeo = (countries.find(c => c.code === 'OS') ? 'OS' : 'Other');
      if (otherGeoEl) otherGeoEl.value = site.otherGeo;
    }
...
function review() {
  ...
  <div><small>${w.category || '—'} • Main: ${w.mainGeo || '—'} (${w.mainGeoShare||0}%) • Secondary: ${w.secondaryGeo || '—'} (${w.secondaryGeoShare||0}%) • Other: ${w.otherGeo || '—'} (${w.otherGeoShare||0}%) • ${w.uniqueUsers || 0} users</small></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  body += `Secondary GEO: ${w.secondaryGeo || '—'} (${w.secondaryGeoShare || 0}%)\n`;
  body += `Other GEO: ${w.otherGeo || '—'} (${w.otherGeoShare || 0}%)\n`;
      ...
      result.innerHTML = `
        ...
        ${state.websites.map((w,i)=>`
          <div class="kpi-card">
            ...
            <div><small>Main: ${w.mainGeo || '—'} (${w.mainGeoShare||0}%) • Secondary: ${w.secondaryGeo || '—'} (${w.secondaryGeoShare||0}%) • Other: ${w.otherGeo || '—'} (${w.otherGeoShare||0}%)</small></div>
...
function addWebsiteCard(preset) {
  ...
  const site = Object.assign({
    ...
    secondaryGeo: '',
    secondaryGeoShare: '',
    otherGeo: 'OS', // set default to "Others"
    otherGeoShare: '',
    desktopShare: '',
    mobileShare: ''
  }, preset || {});
